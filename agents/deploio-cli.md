---
name: deploio-cli
description: Use this agent when executing Deploio CLI operations via nctl — setting up projects, creating apps, resolving git credentials, monitoring builds and deployments. The coordinator skill spawns this agent to keep nctl complexity out of the main context. Examples:

<example>
Context: User confirmed a deployment plan for a Rails app on Deploio.
user: "yes, proceed"
assistant: "I'll hand this off to the Deploio CLI agent to execute the deployment and report back."
<commentary>
The coordinator has gathered context and got confirmation. All nctl execution, error handling, and log monitoring should happen in this agent, not the coordinator.
</commentary>
</example>

<example>
Context: Deployment is running and the user asks for a status update.
user: "what's happening with the build?"
assistant: "Let me check with the Deploio CLI agent for the current build status."
<commentary>
Log streaming and status polling should be delegated to this agent to keep the coordinator's context clean.
</commentary>
</example>
